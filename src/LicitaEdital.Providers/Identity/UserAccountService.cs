...
namespace LicitaEdital.Providers.Identity;
  ...
  : IUserAccountService
{
  ...
  public async Task InvalidateSessionsAsync(UserId userId, CancellationToken cancellationToken = default)
  {
    var user = await _userManager.FindByIdAsync(userId.Value.ToString());
    if (user is null) return;

    // Trocar o `SecurityStamp` invalida cookie ja emitido e token pendente de uma vez — e' o mesmo
    // mecanismo que a troca de senha usa.
